...
interface FormsArtifactProps {
  ...
  onCancel?: () => void;
  submitLabel?: string;
  /** Custom field renderer — FormsArtifact handles state/validation, parent controls visuals */
  renderField?: (field: FormField, value: unknown, onChange: (value: unknown) => void, error?: string) => React.ReactNode;
  /** Custom action buttons renderer */
  renderActions?: (onSubmit: () => void, onCancel?: () => void) => React.ReactNode;
  /** Extra content rendered between fields and actions (e.g. points, attachments) */
  children?: React.ReactNode;
}

export function FormsArtifact({ schema, onSubmit, onCancel, submitLabel = 'Submit', renderField, renderActions, children }: FormsArtifactProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {schema.fields.map(field => (
        renderField
          ? <div key={field.id}>{renderField(field, values[field.id], (v) => setValue(field.id, v), errors[field.id] || undefined)}</div>
      ...
      {renderActions
        ? renderActions(handleSubmit, onCancel)
